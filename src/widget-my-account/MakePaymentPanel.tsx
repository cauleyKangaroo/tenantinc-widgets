...
/* #99's forms, and the stylesheet that draws them. Every rule in it is
   prefixed .rf-/.rf2-, so it cannot reach this widget's own markup. */
import { BankForm, CardForm } from '../widget-rental-flow-2step/PaymentSection';
import '../widget-rental-flow-2step/screens.css';
...
export function MakePaymentPanel({
  ...
  /* Which of the two form-backed methods is open, if either. The wallet
     buttons take no form — they hand off to the wallet sheet — so they are
     not part of this. */
  ...
  return (
    <section className="ma-pay">
      ...
      <div className="ma-methods">
        ...
        {/* Pressing the open one again closes it, so there is always a way out
            of a form that was opened by mistake. */}
        <button
          type="button"
          className={`ma-method ma-method--outline${method === 'card' ? ' ma-method--on' : ''}`}
          aria-expanded={method === 'card'}
          onClick={() => setMethod((m) => (m === 'card' ? null : 'card'))}
        >
          <CreditCardIcon />
        ...
        <button
          type="button"
          className={`ma-method ma-method--outline${method === 'bank' ? ' ma-method--on' : ''}`}
          aria-expanded={method === 'bank'}
          onClick={() => setMethod((m) => (m === 'bank' ? null : 'bank'))}
        >
          <BankIcon />
      ...
      {/* #99's own panel chrome as well as its form, so the two widgets show
          the tenant the same thing. The form carries its own "Pay Now $X",
          which is why the tiles above do not need one. */}
      {method && (
        <section
          className="rf-method-panel ma-payform"
          aria-label={method === 'card' ? 'Credit / Debit' : 'Pay by Bank'}
        >
          <header className="rf-method-panel-head">
            {method === 'card' ? <CreditCardIcon /> : <BankIcon />}
            <span>{method === 'card' ? 'Credit / Debit' : 'Pay by Bank'}</span>
          </header>
          {/* Nothing to submit to while the account API is absent, so Pay Now
              closes the form rather than pretending to take money. */}
          {method === 'card'
            ? <CardForm total={balanceTotal} onPay={() => setMethod(null)} />
            : <BankForm total={balanceTotal} onPay={() => setMethod(null)} />}
        </section>
      )}
    </section>
  );
